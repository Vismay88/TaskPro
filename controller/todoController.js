    ...
    fs.createReadStream(req.file.path)
      ...
      .on('end', async () => {
        ...
        results.forEach((row) => {
          ...
          if(!task.title){
            return (new AppError(`Title is missing on ${task.srno}`,400))
          }
           console.log(task);
          if (task.types === 'main') {
            mainTasks[task.srno] = task;
            // console.log("................")
            console.log("Main task:"+JSON.stringify(mainTasks));
            // console.log("................")
          } else {
        ...
        // Save the main tasks and get their IDs
             const mainTaskIds = [];
        for (const [srno, task] of Object.entries(mainTasks)) {
          const newTask = new Todo({
            title: task.title,
            description: task.description,
            priority: task.priority,
          ...
          const savedTask = await newTask.save();
          mainTasks[srno] = savedTask._id;// Store the task object with its id as well
          mainTaskIds.push(mainTasks[srno]);
          console.log("First one:"+mainTasks[srno]);
        }
        // Assign the subtasks to their main tasks using the corresponding row number
        for (const [srno,subTask]of Object.entries(subTasksof)) {
          const parentTask = mainTasks[subTask.parentId-1];
          console.log(subTask.parentId);
          // console.log("task 1:"+mainTasks[1]) // Get the main task object using its corresponding row number
          console.log("task 2:"+mainTasks[2]);
          // console.log("Below:");
          console.log(parentTask);
